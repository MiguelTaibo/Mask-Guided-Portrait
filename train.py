...
loss_count = 0
# Losses:
# KL_embeb : loss vae encoders (Kullbach-Leibler)
...
for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
    ...
    for i, data in enumerate(dataset, start=epoch_iter):
        ...
        if a_count == 1:    # a_count=1 -> a_weight = 0 cogemos las perdidas de xt=xs
            a_count = 0
            a_weight = 0
        else:               # a_count=0 -> a_weight = 1 cogemos todas las perdidas
            a_count = 1
            a_weight = 1
            
            
        loss_D2 = (loss_dict['D2_fake'] + loss_dict['D2_real']) * 0.5
        loss_G_together = loss_dict['G_GAN']*a_weight \
                          + loss_dict['G_GAN_Feat'] * a_weight \
                          + loss_dict['L2_image'] * 2 * a_weight \
                          + loss_dict['G_VGG'] * 1 * a_weight \
                          + loss_dict['G2_GAN'] \
                          + loss_dict['L2_mask_image'] * 500 \
                          + loss_dict['ParsingLoss']*10
