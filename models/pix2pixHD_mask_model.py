...
class Pix2PixHD_mask_Model(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            if (self.opt.data_type ==16):               #YO
                self.criterionCrossEntropy = torch.nn.CrossEntropyLoss(weight=torch.cuda.HalfTensor(weight_list))
            else:
                self.criterionCrossEntropy = torch.nn.CrossEntropyLoss(weight = torch.cuda.FloatTensor(weight_list))
            ...
            # KL_loss : no se pero es local y no se usa : CriterionKL
            # L2_mask_image : local loss : criterionL2 y lamda local=10
            # G2_GAN (xs!=xt) y G_GAN (xs=xt) : L signmoid (eq 4) from both discriminators : criterioGAN
            # G_GAN_Feat(xs=xt) : L FM : CriterionFeat
            # G_VGG(xs=xt) : no se pero es global y solo si xs=xt: Criterion VGG
            # Discriminators lossese : Not in paper : CriterionGAN
            #   loss_D2_fake, loss_D2_real
            #   loss_D_fake, loss_D_real
            # L2_image(xs = xt) : global loss : CriterionL2 con lambda = [lef_eye=10, right_eye=10, skin=5, hair=5, mouth=10, full = 10]
            # ParsingLoss : global loss from net P masks (not direct output): CrossEntropy Criterion*lamda_feat
            #                                   'no se'     'Local loss''L_sigmoid'   'L_FM'   'no se' 'Discriminator Loss'
    ...
    def forward_vae_net(self, bg_image, label, inst, image, feat, image_affine, mask_list, ori_label, infer=False):
        ...
        # start
        #
        # mask4_image = torch.zeros(label.size()[0],3,int(self.opt.longSize/8),int(self.opt.longSize*6/32)).cuda()
        # mask5_image = torch.zeros(label.size()[0],3,int(self.opt.longSize/8),int(self.opt.longSize*6/32)).cuda()
        # mask_mouth_image = torch.zeros(label.size()[0],3,int(self.opt.longSize*10/32),int(self.opt.longSize*18/32)).cuda()
        # mask_mouth = torch.zeros(label.size()[0],3,int(self.opt.longSize*10/32),int(self.opt.longSize*18/32)).cuda()
        ...
        ##  +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ ##
        if self.opt.data_type==16:
            mask_skin = ((label==1)+(label==2)+(label==3)+(label==6)).type(torch.cuda.HalfTensor)
        else:
        ...
        if self.opt.data_type==16:
            mask_hair = (label==10).type(torch.cuda.HalfTensor)
        else:
            mask_hair = (label == 10).type(torch.cuda.FloatTensor)
        mask_hair_image = mask_hair * real_image

        if self.opt.data_type==16:
            mask_mouth_whole = ((label==7)+(label==8)+(label==9)).type(torch.cuda.HalfTensor)
        else:
            mask_mouth_whole = ((label==7)+(label==8)+(label==9)).type(torch.cuda.FloatTensor)

        for batch_index in range(0,label.size()[0]):
            # print(label.size())
            # print(str(mask4_image.shape)+str(mask5_image.shape)+str(mask_mouth_image.shape) + str(real_image.shape) + str(batch_index) + str(mask_list.shape))
            ...
            # mask4_image[batch_index] = real_image[batch_index, :,
            #                            int(mask_list[batch_index][0]) - int(self.opt.longSize/16):int(mask_list[batch_index][0]) + int(self.opt.longSize/16),
            #                            int(mask_list[batch_index][1]) - int(self.opt.longSize*3/32):int(mask_list[batch_index][1]) + int(self.opt.longSize*3/32)]
            # mask5_image[batch_index] = real_image[batch_index, :,
            #                            int(mask_list[batch_index][2]) - int(self.opt.longSize/16):int(mask_list[batch_index][2]) + int(self.opt.longSize/16),
            #                            int(mask_list[batch_index][3]) - int(self.opt.longSize*3/32):int(mask_list[batch_index][3]) + int(self.opt.longSize*3/32)]
            # mask_mouth_image[batch_index] = real_image[batch_index, :,
            #                                 int(mask_list[batch_index][4]) - int(self.opt.longSize*5/32):int(mask_list[batch_index][4]) + int(self.opt.longSize*5/32),
            #                                 int(mask_list[batch_index][5]) - int(self.opt.longSize*9/32):int(mask_list[batch_index][5]) + int(self.opt.longSize*9/32)]
            #
            # mask_mouth[batch_index] = mask_mouth_whole[batch_index, :,
            #                           int(mask_list[batch_index][4]) - int(self.opt.longSize*5/32):int(mask_list[batch_index][4]) + int(self.opt.longSize*5/32),
            #                           int(mask_list[batch_index][5]) - int(self.opt.longSize*9/32):int(mask_list[batch_index][5]) + int(self.opt.longSize*9/32)]
        ...
        #if self.opt.data_type==16:
        #    input_label = input_label.to(device = self.device, dtype = torch.half, non_blocking=False, copy=False)
        ...
        if self.opt.data_type==16:
            mask_bg = (label==0).type(torch.cuda.HalfTensor)
        else:
        ...
        #+++++++++++++++++++++++++++ KL_loss: No se
        #+++++++++++++++++++++++++++ loss_mask_image: Local Loss (multiplicada por 10 (lambda local))
        ...
        #End KL_loss and loss_mask_image
...
# insert unpaired part
        ...
        # L gp (eq. 8) y lambda_feat : lambda gp (ParssingLoss)
        loss_parsing = self.criterionCrossEntropy(parsing_label_feature,gt_label)*self.opt.lambda_feat

        # Observar las entradas y ver la funcion de loss: D2 real y fake: LDi (equ. 3)
...
########################################################################################################
        ...
        # L fm (eq. 5) lambda_fet = lambda fm
        loss_G_GAN_Feat = 0
    ...
    def inference(self, bg_contentimage, label2, mask2_list, image, label, mask_list):
        ...
        with torch.no_grad():
            ...
            mask4_image = torch.zeros(label.size()[0],3,int(self.opt.longSize/8),48).cuda()
            mask5_image = torch.zeros(label.size()[0],3,int(self.opt.longSize/8),48).cuda()
            mask_mouth_image = torch.zeros(label.size()[0],3,80,144).cuda()
            mask_mouth = torch.zeros(label.size()[0],3,80,144).cuda()            

            # label == 1 : cara
            # label == 2 : ceja derecha
            # label == 3 : ceja izquierda
            # label == 6 : nariz
            mask_skin = ((label==1)+(label==2)+(label==3)+(label==6)).type(torch.cuda.FloatTensor)
            mask_skin_image = mask_skin * real_image
            # label == 10 : pelo
            mask_hair = (label==10).type(torch.cuda.FloatTensor)
            mask_hair_image = mask_hair * real_image
            # label == 7 : labio superior, label == 8 : interior boca, label == 9 : labio inferior
            mask_mouth_whole = ((label==7)+(label==8)+(label==9)).type(torch.cuda.FloatTensor)


            bg_content_feature = self.netG.forward(real_content_image,type="bg_encoder") #xt-- Backgroun fusing subnetwork
            mask_content_bg = (label2==0).type(torch.cuda.FloatTensor)
            mask_content_bg_feature = mask_content_bg * bg_content_feature

            bg_style_feature = self.netG.forward(real_image,type="bg_encoder") #xs -- Backgroun fusing subnetwork
            ...
            #Mask guided generative subnetwork
            encode_label_feature = self.netG.forward(input_label,type="label_encoder") #ms
            encode_label2_feature = self.netG.forward(input_label2,type="label_encoder") #mt

            #Local embeding subnetwork
            if self.opt.random_embed == False:
                ...
                with torch.no_grad():
                    mus4, log_variances4 = self.net_encoder_left_eye(mask4_image) #left eye
                    correct_sample4 = Variable(torch.randn(mus4.size()).cuda(), requires_grad=True)
                    decode_embed_feature4 = self.net_decoder_left_eye(correct_sample4)

                    mus5, log_variances5 = self.net_encoder_right_eye(mask5_image) #right eye
            ...
            #G_m de Mask-Guided generative subnetwork del source
            ...
            #G_m de Mask guided generative subnetwork pero del target
            ...
            #G_b salida [1,3,256,256] background fusing
            reconstruct_fake_style = self.netG.forward(torch.cat((fake_style_face,mask_style_bg_feature),1),type="bg_decoder") #pasa de source a source: comprobacion de la red
            reconstruct_fake_content = self.netG.forward(torch.cat((fake_content_face,mask_content_bg_feature),1),type="bg_decoder") #cambio de source a target: red real
...
#        assert torch.__version__.startswith('0.4')
    ...
    def inference_parsing(self, label, image):
        # Encode Inputs        
        _, _, real_image, _, _ = self.encode_input(Variable(label), None, Variable(image), infer=True) #input_label
